...
def farthest_point_sample(xyz, npoint):
    """
    :function: sampled the points

    :param xyz: pointcloud data, [B, N, 3]
    :param npoint: npoint: number of samples
...
class TopoPointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(TopoPointMLP, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = num_classes
        self.points = points
        self.embedding = ConvBNReLU1D(6, embed_dim, bias=bias, activation=activation)
        self.topologyfeature = TopologyFeature(embed_dim, bias, activation, *init_parameters_test())

        #self.embedding_topo_geome = ConvBNReLU1D(embed_dim * 2, embed_dim, bias=bias, activation=activation)
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(gmp_dim+cls_dim+de_dims[-1]+embed_dim, 128, 1, bias=bias),
            nn.BatchNorm1d(128),
            nn.Dropout(),
    ...
    def forward(self, x, norm_plt, cls_label, pd):
        xyz = x.permute(0, 2, 1)
        _, N, _ = xyz.shape

        x = torch.cat([x,norm_plt],dim=1)
        topofeature = self.topologyfeature(xyz, pd)

        x = self.embedding(x)  # B,D,N
        #x = self.embedding_topo_geome(torch.concat((x, topofeature.repeat((1, 1, N))), dim=1))  # B,D,N
        ...
        x = torch.cat([x, global_context.repeat([1, 1, x.shape[-1]]), cls_token.repeat([1, 1, x.shape[-1]]), topofeature.repeat([1, 1, x.shape[-1]])], dim=1)
        x = self.classifier(x)
...
def topoPointMLP(num_classes=50, **kwargs) -> TopoPointMLP:
    return TopoPointMLP(num_classes=num_classes, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
...
if __name__ == '__main__':
    ...
    model = topoPointMLP(50)
    out = model(data, norm,cls_label, pd)  # [2,2048,50]
    print(out.shape)
