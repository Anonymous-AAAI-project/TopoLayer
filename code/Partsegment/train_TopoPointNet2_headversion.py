...
from util.data_util import PartNormalDataset, Collatet
import torch.nn.functional as F
import torch.nn as nn
import model.TopoPointNet2_headversion as models
import numpy as np
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt, pd) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        seg_pred = model(points, norm_plt, to_categorical(label, num_classes), pd)  # seg_pred: b,n,50
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt, pd) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        batch_size, num_point, _ = points.size()
        points, label, target, norm_plt = Variable(points.float()), Variable(label.long()), Variable(target.long()), \
                                          Variable(norm_plt.float())
        points = points.transpose(2, 1)
        ...
        points, label, target, norm_plt = points.cuda(non_blocking=True), label.squeeze(1).cuda(non_blocking=True), \
                                          target.cuda(non_blocking=True), norm_plt.cuda(non_blocking=True)
        seg_pred = model(points, norm_plt, to_categorical(label, num_classes), pd)  # b,n,50
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='PointNet2PartSeg')
    parser.add_argument('--exp_name', type=str, default='demo6', metavar='N',
