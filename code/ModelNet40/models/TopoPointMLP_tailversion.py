...
def square_distance(src, dst):
    """
    :function: Calculate Euclid distance between each two points.
...
def index_points(points, idx):
    """
    :function: get the points of sample index

    :param points: input points data, [B, N, C]
    :param idx: sample index data, [B, [/npoints], S]

    :return new_points: indexed points data, [B, [/npoints], S, C]
    """
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = torch.cat([grouped_points, new_points.view(B, S, 1, -1).repeat(1, 1, self.kneighbors, 1)],
                               dim=-1)
...
class TopologyFeature(nn.Module):
    ...
    def forward(self, xyz, pd):
        ...
        return self.mlp(topo_feature.permute(0, 2, 1))


class ModelWithPD(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1,
                 activation="relu", bias=True, use_xyz=True, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[2, 2, 2, 2], **kwargs):
        super(ModelWithPD, self).__init__()
        ...
        #self.embedding_topo_geome = ConvBNReLU1D(last_channel * 2, last_channel, bias=bias, activation=activation)
        self.act = get_activation(activation)
        self.classifier = nn.Sequential(
            nn.Linear(last_channel+64, 512),
            nn.BatchNorm1d(512),
            self.act,
            nn.Dropout(0.5),
            nn.Linear(512, 256),
    ...
    def forward(self, x, pd):
        ...
        #x = self.embedding_topo_geome(torch.concat((x, self.topologyfeature(xyz, pd).to(device)), dim=1))  # B,D,N
        x = torch.concat((x, self.topologyfeature(xyz, pd)), dim=1)  # B,D,N        
        x = x.squeeze(dim = -1)
        x = self.classifier(x)
        return x


def topoPointMLP(num_classes=40, **kwargs):
    return ModelWithPD(points=1024, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
