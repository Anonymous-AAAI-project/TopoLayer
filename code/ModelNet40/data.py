...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition = "train", opt = "delay"):
        """
        ...
        :return realtime: pointcloud, label
                delay: pointcloud, pd_pointcloud(tuple,each element is tensor), label
                offline: pointcloud, pd_pointcloud(tuple,each element is tensor), label
        """
        self.opt = opt
        if opt == "realtime":
            ...
            for data in tqdm(self.data):
                pointcloud = data[:self.num_points]
                self.pd_data.append(PDDataStructure(pointcloud))
        elif opt == "offline":
            self.data, self.label = load_data(partition)
            ...
            raise Exception("opt should be realtime, delay of offline")
...
class Collatet():
    ...
    def __call__(self, batch):
        if self.opt == "delay":
            ...
            processed_pd_data = [item[1].pd_data for item in batch]
        ...
        elif self.opt == "realtime":
            processed_pointcloud = torch.utils.data.dataloader.default_collate(batch)
            return processed_pointcloud

        elif self.opt == "offline":
            processed_pointcloud = torch.utils.data.dataloader.default_collate([item[0] for item in batch])
            processed_pd_data = [(torch.tensor(item[1][0], dtype = torch.float32), torch.tensor(item[1][0], dtype = torch.float32)) for item in batch]
            processed_label = torch.utils.data.dataloader.default_collate([item[2] for item in batch])
            return processed_pointcloud, processed_pd_data, processed_label
